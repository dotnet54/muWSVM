...
public class WSVMPanel extends ChartPanel 
						implements ChartMouseListener, ISubject, IObserver{
	...
	private WSVMDataSet chartData = null;
	...
	private WSVMDataItem selectedDataItem = null;
    ...
    private class LabelGenerator implements XYItemLabelGenerator {
        ...
        public String generateLabel(XYDataset dataset, int series, int item) {
            if (dataset instanceof WSVMDataSet){
            	WSVMDataSet svmData = (WSVMDataSet) dataset;
            	WSVMDataSeries svmSeries =  svmData.getSeries(series);
            	WSVMDataItem svmItem = svmSeries.getDataItem(item);
    ...
    /**
     * a controller class, which is a container to chart
     * this class also communicates with model and handle mouse input
     * 
     * @param chart
     * @param model
     */
	public WSVMPanel(JFreeChart chart,WSVMModel model ) {
		super(chart);
		...
		setupDatasetsAndRenderers();
		
        model.register(this);
	}

	
	private void setupDatasetsAndRenderers(){
		//Dataset settings
...
//		XYDotRenderer trainingDatasetRenderer = new XYDotRenderer();
//		trainingDatasetRenderer.setDotWidth(4);
//		trainingDatasetRenderer.setDotHeight(4);
//		thisPlot.setRenderer(trainingDatasetRenderer);
	}
	...
	public void updateWRCHSolutions(){
		...
		ArrayList<WSVMDataItem> rch1 = model.getRCH1();
		ArrayList<WSVMDataItem> rch2 = model.getRCH2();
	...
	public void updateWSVMSolutions(){
		try {
			...
			WSVMDataItem w = model.getW();
			double b = model.getB();
			WSVMDataItem p =  model.getNearestPositivePoint();
			WSVMDataItem n = model.getNearestNegativePoint();			
	...
	public void drawLine(WSVMDataItem vector){
		try {
			thisPlot.clearAnnotations();
	...
	private void initPopUpMenu(){
		...
		itu.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					WSVMDataItem newPoint = new WSVMDataItem(model.getTestData().getDimensions());
		...
		itw.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent ev) {
	            ...
	            if (dataset instanceof WSVMDataSet){
	            	WSVMDataSet svmData = (WSVMDataSet) dataset;
	            	WSVMDataSeries series =  svmData.getSeries(e.getSeriesIndex());
	            	WSVMDataItem item = series.getDataItem(e.getItem());
	...
	public WSVMDataItem getSelectedDataItem(){
			...
			for (int j = 0; j < list.size(); j++){
				...
				if (ce instanceof XYItemEntity) {
		            ...
		            if (e.getArea().contains(x, y)){
			            ...
			            WSVMDataItem item  = (WSVMDataItem) ss.getItems().get(i);
			            
			            if (d.equals(model.getTrainingData())
			            	&&	s == 0 
	...
	public void addPoint(int series, WSVMDataItem newPoint) throws Exception{
		int xDim = model.getTrainingData().getXDimension();
		int yDim = model.getTrainingData().getYDimension();
		
		newPoint.setVal(xDim, xChart);
		newPoint.setVal(yDim, yChart);
		model.getTrainingData().addItem(series, newPoint);
	}
    ...
    public void clearWRCH(){
    	model.getSolutionDataSet().getSeries(0).clear();
    	model.getSolutionDataSet().getSeries(1).clear();
    	model.getSolutionDataSet().getSeries(2).clear();
